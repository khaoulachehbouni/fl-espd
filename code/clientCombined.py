...
parser.add_argument(
  "--model_version",
  dest='model_version',
  help="Version of the model", #name of the model
  required=True
)
parser.add_argument(
  "--output_dir",
...
parser.add_argument(
  "--warmup_rows",
  dest = "warmup_rows",
  help = "the number of rows from each label i want to sample from the warm-up data",
  type = int,
  default = 10
)
...
parser.add_argument(
  "--neg_conv",
  dest = "neg_conv",
  help = "number of negative conversations i want to merge",
  type = float,
  default = 10
)
...
def client_fn(cid: str) -> fl.client.Client:
    ...
    df_warmup_pred = df_warmup_all.query('(labels==1)').sample(n=args.warmup_rows)
    df_warmup_nonpred = df_warmup_all.query('(labels==0)').sample(n=args.warmup_rows)
    ...
    if df_user['labels'].values.any() == 0:
        add_neg = random.choices(train_nonpred_list, k=int(args.neg_conv))
...
def get_eval_fn(model: LogisticRegression):
    ...
    def evaluate(server_round: int, parameters: NDArrays, config: Dict[str, Scalar]) -> Optional[Tuple[float, Dict[str, Scalar]]]:
        ...
        name_model = 'model-%s-c%s-ff%s-r%s-wu%s-nc%s.sav' % (args.model_version, args.num_clients, args.fraction_fit, args.num_rounds, args.warmup_rows, args.neg_conv)
        model_file = os.path.join(args.output_dir, name_model)
        pickle.dump(model, open(model_file, 'wb'))
        name_results = 'results-%s-c%s-ff%s-r%s-wu%s-nc%s.txt' % (args.model_version, args.num_clients, args.fraction_fit, args.num_rounds, args.warmup_rows, args.neg_conv)
...
strategy = fl.server.strategy.FedAvg(
    ...
    min_available_clients=int(args.num_clients*0.50),  # Wait until at least 75% clients are available
