...
out_dir = "/models/%s/message_based_eval/" % (args.model_name) #add path to model folder
Path(out_dir).mkdir(parents=True, exist_ok=True) # might not exist yet

datapack_path = "/models/%s/message_based_eval/annotated-datapack-PANC-test-eval_mode-segments_fast--window_size-50.json" % (args.model_name)  #add path to model folder
...
datapack_path = "/models/%s/message_based_eval/annotated-datapack-PANC-test-eval_mode-full_fast--window_size-50.json" % (args.model_name)  #add path to model folder
with open(datapack_path, "r") as file:
